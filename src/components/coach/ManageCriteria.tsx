...
export const ManageCriteria: React.FC = () => {
  ...
  const [assignedSports, setAssignedSports] = useState<AssignedSport[]>([]);
  ...
  // --- CRUD Fetch Functions (Declared first to avoid hoisting warnings) ---
  const fetchAssignedSports = async () => {
    ...
    try {
      ...
      if (list.length > 0) {
        // Pre-select sport from URL param if present and valid, otherwise default to first
  ...
  // --- Effects ---
  // Re-run when the URL sportId changes (user clicks a different sport in the sidebar)
  useEffect(() => {
    fetchAssignedSports();
  }, [urlSportId]);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-slate-400">Design dynamic evaluation templates for your assigned clubs.</p>
        </div>
      ...
      {assignedSports.length === 0 ? (
        <div className="glass-panel rounded-xl p-8 text-center text-slate-400">
          No sports clubs assigned to your coach account yet. Please contact your Head Coach to gain access.
        </div>
      ) : (
